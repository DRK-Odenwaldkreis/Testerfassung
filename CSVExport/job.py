...
if __name__ == "__main__":
    try:
        ...
        if len(sys.argv) == 2:
            ...
            if not station:
                station = 0
            sql = "Select id,Nachname,Vorname,Geburtsdatum,Adresse,Telefon,Mailadresse,Ergebnis,Ergebniszeitpunkt,Teststation from Vorgang where Teststation = %s and Ergebniszeitpunkt Between '%s 00:00:00' and '%s 23:59:59';" % (station,
            requestedDate.replace('-', '.'), requestedDate.replace('-', '.'))
            gesundheitsamt=False
        elif len(sys.argv) == 4:
            requestedDate = sys.argv[1]
